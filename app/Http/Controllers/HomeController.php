...
class HomeController extends Controller
{
    ...
    public function productDetails($slug, Request $request)
    {
        ...
        $defaultDressStyleId = $cheapestVariant->dress_style_id ?? null;
        $defaultSize = $cheapestVariant->size ?? null;

        // Get available dress styles with their names
        $availableDressStyles = $product->variants
            ->whereNotNull('dress_style_id')
            ->pluck('dressStyle.name', 'dress_style_id')
            ->unique();
        ...
        // Get product images (avatar + gallery images)
        ...
        foreach ($product->images as $image) {
            $productImages->push($image->image_path);
        }

        // If no images, use a default placeholder
        if ($productImages->isEmpty()) {
            $productImages->push('assets/images/default-product.jpg');
        }
        ...
        $formattedVariants = $product->variants->map(function ($variant) {
            return [
                'id' => $variant->id,
                'dress_style_id' => $variant->dress_style_id,
                'dress_style_name' => $variant->dressStyle->name ?? '',
                'size' => $variant->size,
                'price' => (float)$variant->price,
                'discounted_price' => (float)$variant->getDiscountedPrice(),
                'sku' => $variant->sku,
            ];
        });
        ...
        $productData = [
            ...
            'dress_styles' => $availableDressStyles->toArray(),
            'all_sizes' => $allSizes,

            'default_dress_style_id' => $defaultDressStyleId,
    ...
    private function applyProductFilters($query, Request $request)
    {
        ...
        if ($request->has('dress_styles') && !empty($request->dress_styles)) {
            $styleIds = is_array($request->dress_styles) ? $request->dress_styles : explode(',', $request->dress_styles);
            $query->whereHas('variants', function ($q) use ($styleIds) {
                $q->where('status', ProductVariant::STATUS_ACTIVE)
                    ->whereIn('dress_style_id', $styleIds);
            });
        }
        ...
        if ($request->has('sizes') && !empty($request->sizes)) {
            ...
            $query->whereHas('variants', function ($q) use ($sizes) {
                $q->whereIn('size', $sizes)->where('status', ProductVariant::STATUS_ACTIVE);
            });
        }

        // Filter theo price range with discount consideration
        if ($request->has('price_min') && $request->has('price_max')) {
            ...
            if ($minPrice > 0 || $maxPrice < 100000000) {
                $query->whereHas('variants', function ($q) use ($minPrice, $maxPrice) {
                    $q->where('status', ProductVariant::STATUS_ACTIVE)
                        ->where(function ($subQuery) use ($minPrice, $maxPrice) {
                            // Check both original price and discounted price
                            $subQuery->whereBetween('price', [$minPrice, $maxPrice])
                                ->orWhereRaw("
                                       EXISTS (
                                           SELECT 1 FROM product_variant_promotions pvp
                                           JOIN promotions p ON pvp.promotion_id = p.id
                                           ...
                                           AND (
                                               CASE
                                                   WHEN p.type = 'percentage' THEN
                                                       product_variants.price * (1 - p.value / 100)
                                                   ELSE
                                                       product_variants.price - p.value
                                               END
                                           ) BETWEEN ? AND ?
                                       )
                                   ", [$minPrice, $maxPrice]);
        ...
        if ($request->has('sort')) {
            switch ($request->sort) {
                case 'price-low':
                    $query->orderBy(\DB::raw('(
                        SELECT MIN(
                            CASE
                                WHEN EXISTS (
                                    SELECT 1 FROM product_variant_promotions pvp2
                                    JOIN promotions p2 ON pvp2.promotion_id = p2.id
                                    WHERE pvp2.product_variant_id = pv.id
                                    AND p2.status = true
                                    ...
                                    SELECT MIN(
                                        CASE
                                            WHEN p2.type = "percentage" THEN pv.price * (1 - p2.value / 100)
                                            ELSE pv.price - p2.value
                                        END
                                    )
                                    FROM product_variant_promotions pvp2
                                    JOIN promotions p2 ON pvp2.promotion_id = p2.id
                                    WHERE pvp2.product_variant_id = pv.id
                                    AND p2.status = true
                                    AND p2.start_date <= NOW()
                ...
                case 'most-popular':
                default:
                    $query->withCount(['productViews as total_views' => function ($query) {
    ...
    private function getAppliedFilters(Request $request)
    {
        ...
        if ($request->has('dress_styles')) {
            $filters['dress_styles'] = is_array($request->dress_styles) ? $request->dress_styles : explode(',', $request->dress_styles);
        }
